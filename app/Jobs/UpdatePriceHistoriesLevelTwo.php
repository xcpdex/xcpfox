...
class UpdatePriceHistoriesLevelTwo implements ShouldQueue
{
    ...
    public function handle()
    {
        try
        {
            ...
            // Order Match Daily Summaries (Backward)
            $backward = \App\OrderMatch::where('backward_asset', '=', $base_asset->asset_name)
                ->where('quality_score', '=', 1)
                ->selectRaw('DATE(confirmed_at) as date,
                             SUM(backward_quantity) as backward_quantity,
                             SUM(backward_quantity_usd) as backward_quantity_usd,
            ...
            // Iterate Through Days
            foreach($backward as $b)
            {
                // Order Match Summary Today (Forward)
                $f = \App\OrderMatch::where('forward_asset', '=', $base_asset->asset_name)
                    ->where('confirmed_at', 'like', $b->date . '%')
                    ->where('quality_score', '=', 1)
                    ->selectRaw('SUM(backward_quantity) as backward_quantity,
                                 SUM(backward_quantity_usd) as backward_quantity_usd,
                                 SUM(forward_quantity) as forward_quantity,
                ...
                \App\PriceHistory::firstOrCreate([
                    'ticker' => $base_asset->asset_name,
                    'price' => $price,
                    'timestamp' => \Carbon\Carbon::createFromFormat('Y-m-d', $b->date, 'America/New_York')->timestamp,
                    'quality_score' => 2,
                ],[
                    'confirmed_at' => \Carbon\Carbon::createFromFormat('Y-m-d', $b->date, 'America/New_York'),
                ]);
            }
        }
        catch(\Exception $e)
