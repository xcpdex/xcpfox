...
@section('title', 'Cumulative Counterparty Fees (USD)')

@section('header')
<link rel="canonical" href="{{ url(route('charts.cumulative-fees-usd')) }}" />
    ...
    $(function () {
        ...
        $.getJSON('{{ url(route('api.charts.total-fees-usd', ['group_by' => $group_by])) }}', function(data) {
            ...
            $('#chart').highcharts({
                ...
                title: {
                    text: "Cumulative Counterparty Fees (USD)"
                },
                subtitle: {
    ...
    <div class="row">
        <div class="col-md-4 mt-3">
            <div class="btn-group d-flex" role="group" aria-label="Chart">
                <a href="{{ url(route('charts.cumulative-fees', ['chart' => $chart, 'group_by' => $group_by])) }}" role="button" class="btn btn-sm btn-outline-primary w-100">BTC</a>
                <a href="{{ url(route('charts.cumulative-fees-usd', ['chart' => $chart, 'group_by' => $group_by])) }}" role="button" class="btn btn-sm btn-outline-primary w-100 active">USD</a>
            </div>
        </div>
        <div class="col-md-4 mt-3">
